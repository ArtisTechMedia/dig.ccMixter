...
CCC.Upload = Em.ObjectProxy.extend({
  ...
  isLicenseReady: function() {
    return ((this.get('upload_tags') || '').indexOf('ccplus') !== -1);
  ...
  isRecommended: function() {
    return (this.get('ccc.recommends') || []).contains(this.get('upload_id'));
  }.property('ccc.recommends.@each', 'upload_id'),

  isOwnUpload: function() {
    return (this.get('user_name') === this.get('ccc.username'));
